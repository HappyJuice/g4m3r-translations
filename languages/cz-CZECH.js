			...
			COMMANDS_SERVER: {
				...
				settingsValue: settings =>
					[
						...
						`Automaticky přidělené role: ${settings.settings.autoAssignRoles.mainRole ? `<@&${settings.settings.autoAssignRoles.mainRole}>` : ''}`,
						`Samostatně přidělitelné role: ${settings.saRoles.length ? settings.saRoles.map(roleID => `<@&${roleID}>`).join(' ') : 'Žádné'}`,
