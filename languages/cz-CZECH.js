...
module.exports = class extends Language {
	...
	constructor(...args) {
		...
		this.language = {
			...
			COMMANDS_SERVER: {
				title: guildName => `Serverové statistiky pro « ${guildName} »`,
				...
				settingsName: 'G4M3R - Základní nastavení',
                settingsValue: settings =>
                    [
                        ...
                        `Možnosti zpětné vazby: **Návrhy:** ${settings.settings.feedback.idea.status} | **Problémy**: ${settings.settings.feedback.bug.status}`
                    ].join('\n'),
                moderationName: 'G4M3R - Nastavení moderování',
                moderationValue: settings =>
                    [
                        `Automaticky přiřazené role: ${settings.settings.autoAssignRoles.mainRole ? `<@&${settings.settings.autoAssignRoles.mainRole}>` : ''}`,
                        `Samostatně přiřaditelné role: ${settings.saRoles.length ? settings.saRoles.map(roleID => `<@&${roleID}>`).join(' ') : 'Žádné'}`,
                        `Přiřaditelné role moderátorem: ${settings.assignRoles.length ? settings.assignRoles.map(roleID => `<@&${roleID}>`).join(' ') : 'Žádné'}`,
                        `Ověřovací role: ${settings.settings.verify.role ? `<@&${settings.settings.verify.role}>` : 'Žádné'}`,
                        `Mail kategorie: ${settings.settings.mail.mailCategory ? settings.settings.mail.mailCategory : 'Žádná'}`,
                        `Ověřovací kategorie: ${settings.settings.verify.category}`,
                        `Kanály pro ověření: ${settings.welcomeChannels.length}`
			...
			EVENTS_ADD_ROLE_QUESTION: 'Uveď prosím @role, ID rolí nebo názvy rolí. Všichni uživatelé s těmtito rolemi budou přidáni do eventu.',
			EVENT_PLATFORM_QUESTION: 'Na které platformě se event uskuteční?',
			FEEDBACK_DEFAULT: action => `**${klasaUtil.toTitleCase(action)}** - zpětná vazba`,
			FEEDBACK_EDIT_NOT_FOUND: id => `Nepodařilo se mi najít znění zpětné vazby s ID: ${id}`,
			FEEDBACK_EDIT_NOT_AUTHOR: 'Tuto zpětnou vazbu nemůžeš upravit, protože nejsi její autor.',
			FEEDBACK_EDIT_SUCCESS: 'Zpětná vazba byla upravena.',
			FEEDBACK_ORIGINAL_NOT_FOUND: 'Původní znění zpětné vazby nebylo nalezano, takže ho nelze upravit.',
			FEEDBACK_FOOTER: (prefix, action, id) => `Pro odeslání zpětné vazby: ${prefix}fb ${action} | Pro upravení zpětné vazby: ${prefix}fb edit ${id}`,
			FEEDBACK_FROM: (type, authorName) => `${klasaUtil.toTitleCase(type)} od ${authorName}`,
			FEEDBACK_NOT_ACTIVATED:
				'Tento server nemá aktivovanou možnost posílat zpětnou vazbu. Kontaktuj prosím administrátora nebo moderátora serveru, aby tuto funkci povolili.',
			...
			COMMANDMESSAGE_MISSING_REQUIRED: name => `${name} is a requ
